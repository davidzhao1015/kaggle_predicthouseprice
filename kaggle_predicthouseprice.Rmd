...
# identify numeric variables with distinct values less than 10 
var_unique <- sapply(train_df_id, function(x) length(unique(x))) %>% 
    as.data.frame() %>% 
    rownames_to_column("variables")
  
colnames(var_unique) <- c("variables", "length") 


# extract variables with unique values more than 10 - numeric variables 
is_num_var <- var_unique %>% 
  filter(length > 10) %>% 
  pull(variables)
...
```
27 numeric variables other than the response variable `SalePrice`. 


```{r category variable}

# identify categorical variables with no more than 10 unique values 
is_fact_var <- var_unique %>% 
  filter(length <= 10) %>% 
  pull(variables)
...
dim(dummies_train_df)  # 290 variables 
...
# drop top 10 variables with substantial missing values 
dummies_train_missing <- dummies_train_nzv2 %>% 
  select(-all_of(top10_var_missing)) 

head(dummies_train_missing)
...
# subset numeric variables
dummies_train_missing_num <- dummies_train_missing %>%
        dplyr::select(where(is.numeric))

# pick top numeric variables based on rho
res_num <- cor(dummies_train_missing_num, method = "spearman")
...
# gbm_train <- train(SalePrice ~ .,
#                    data = dummies_train_missing3,
#                    method = "gbm_h2o", 
#                    trControl = fitControl)
# 
# gbm_train # model information 
...
# # add predicted data to the train set 
# dummies_train_missing3$pred_lm <- predict(lm_train) # lm 
# dummies_train_missing3$pred_glm <- predict(glm_train) # glm 
# dummies_train_missing3$pred_gam <- predict(gam_train)  # gam  
# dummies_train_missing3$pred_rf <- predict(rf_train)  # rf 
# 
# 
...
# pred_truth_df <- dummies_train_missing3 %>% 
...
# random forest 
# GainCurvePlot(dummies_train_missing3, "pred_rf", "SalePrice", "Random Forest model for home price")   

# 
# # lm 
# GainCurvePlot(dummies_train_missing3, "pred_lm", "SalePrice", "Linear model for home price")  
...
# rf_perf <- postResample(pred = dummies_train_missing3$pred_rf, obs = dummies_train_missing3$SalePrice) %>% as.data.frame() # rf 
# 
# lm_perf <- postResample(pred = dummies_train_missing3$pred_lm, obs = dummies_train_missing3$SalePrice) %>% as.data.frame() # lm 
# 
# gam_perf <-  postResample(pred = dummies_train_missing3$pred_gam, obs = dummies_train_missing3$SalePrice) %>% as.data.frame()  # gam 
# 
# glm_perf <-  postResample(pred = dummies_train_missing3$pred_glm, obs = dummies_train_missing3$SalePrice) %>% as.data.frame() # glm 
# 
# 
...
#            glm = glm_perf) 
# 
# colnames(merg_perf) <- c("rf", "lm", "gam", "glm")
# 
# merg_perf <- merg_perf %>% 
...
factor_test <- sapply(test_df2, function(x) length(levels(x))) %>% 
  as.data.frame() %>% 
  rownames_to_column("variable") 
...
# convert factor variables into dummy variables (i.e., one-hot-encoding)
dummies_test <- dummyVars(~.,   # formula excluding a one-level factor 
                          data = test_df3, 
                          fullRank = TRUE,
                          sep = "_") 

dummies_test # 42 factors


# dummy variables created 
dummies_test_df <- data.frame(predict(dummies_test, newdata = test_df3)) 
...
dummies_test_df_na <- dummies_test_df %>% replace(is.na(.), 0) 

# predict on the test set 
dummies_test_df_na$SalePrice_pred_rf <- predict(rf_train, dummies_test_df_na)  
